<form action="{{ route('co.create-bill', ['id' => $order->id, 'partner' => 'ghn']) }}" method="POST" id="ghn-form" class="cod-form">
    {{ csrf_field() }}
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">Đẩy đơn hàng lên GHN</h4>
    </div>
...
<script>
    let ghnXhrPools = [];
    function ghnAbortRequest() {
        ghnXhrPools.map(function(jqXHR) {
            jqXHR.abort();
        });
        ghnXhrPools = [];
    }

    function getServices(shopId, fromDistrict, toDistrict)
    {
        return $.ajax({
            type: 'GET',
            ...
            beforeSend: function(jqXHR) {
                $('#ghn-form .transfer-info select[name="service_id"]').empty();
            },
            dataType: 'JSON',
            success: function(data) {
                for ([id, value] of Object.entries(data)) {
                    $('#ghn-form .transfer-info select[name="service_id"]').append("<option value='" + id + "'>" + value + "</option>");
                }
            }
        })
    }

    function ghnGetPrice(el)
    {
        const formData = $('#ghn-form').serializeObject();
        var senderDistrict = $('#ghn-form .inventory option:selected').data('district');
        if (el && ['inventory', 'to_district_id'].indexOf(el.attr('name')) !== -1) {
            getServices(formData.inventory, senderDistrict, formData.to_district_id)
                .then(() => {
                    $('#ghn-form .transfer-info select[name="service_id"]').change();
                });
        }
        $.ajax({
            type: 'GET',
            url: '{{ route("co.get-price", ["partner" => $partner]) }}',
            data: {
            ...
            beforeSend: function(jqXHR) {
                ghnAbortRequest();
                ghnXhrPools.push(jqXHR);
                $('#ghn-form input[name="fee_total"]').val('0 đ');
            },
            success: function(res) {
                $('#ghn-form input[name="fee_total"]').val(res.toLocaleString() + ' đ');
            }
        });
    }
$(function() {
    if ($('#ghn-form .partner-province').val() && $('#ghn-form .partner-district').val()) {
        const formData = $('#ghn-form').serializeObject();
        var senderDistrict = $('#ghn-form .inventory option:selected').data('district');
        getServices(formData.inventory, senderDistrict, formData.to_district_id)
            .then(() => {
                if ('{{ @$order->cod_service_id }}') {
                    $('#ghn-form .transfer-info select[name="service_id"]').val('{{ $order->cod_service_id }}').change();
                } else {
                    $('#ghn-form .transfer-info select[name="service_id"]').change();
                }
                ghnGetPrice();
    ...
    $('#ghn-form .getprice-required').change(function() {
        ghnGetPrice($(this));
    });
});
</script>
