...
class RolesController extends Controller
{
	...
	{
		$values = Role::select($this->listing_cols)->whereNull('deleted_at');
		$out = Datatables::of($values)->make();
		$data = $out->getData();

		$fields_popup = ModuleFields::getModuleFields('Roles');

		$roles = Role::whereIn('id', array_map(function($value) { return $value[0]; },$data->data))->get();
		for($i=0; $i < count($data->data); $i++) {
            $role = $roles->where('id', $data->data[$i][0])->first();
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				$col = $this->listing_cols[$j];
				if($fields_popup[$col] != null && starts_with($fields_popup[$col]->popup_vals, "@")) {
					$data->data[$i][$j] = ModuleFields::getFieldValue($fields_popup[$col], $data->data[$i][$j]);
				}
				if($col == $this->view_col && $role->store_id == auth()->user()->store_id) {
