...
class DatabaseSeeder extends Seeder
{
	...
	public function run()
	{
		...
		// Create Super AdminPanel Role
		$role = new Role;
		$role->name = "SUPER_ADMIN";
		$role->display_name = "Super AdminPanel";
		$role->description = "Full Access Role";
		$role->parent = 1;
		$role->dept = $dept->id;
		$role->save();
		
		// Set Full Access For Super AdminPanel Role
		foreach ($modules as $module) {
			Module::setDefaultRoleAccess($module->id, $role->id, "full");
		}
		
		// Create AdminPanel Panel Permission
		$perm = new Permission;
		$perm->name = "ADMIN_PANEL";
		$perm->display_name = "AdminPanel Panel";
		$perm->description = "AdminPanel Panel Permission";
		$perm->save();
		...
		$laconfig->value = "LaraAdmin is a open-source Laravel AdminPanel Panel for quick-start AdminPanel based applications and boilerplate for CRM or CMS systems.";
		$laconfig->save();
